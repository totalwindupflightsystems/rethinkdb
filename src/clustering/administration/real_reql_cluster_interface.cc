...
bool real_reql_cluster_interface_t::subscription_create(
        auth::user_context_t const &user_context,
        counted_t<const ql::db_t> db,
        const name_string_t &table,
    ...
    try {
        ...
        *error_out = admin_err_t{
            strprintf("Subscription `%s` already exists on table `%s.%s`.",
                      config.name.c_str(), db->name.c_str(), table.c_str()),
            query_state_t::FAILED};
        return false;
    } CATCH_NAME_ERRORS(db->name, table, error_out)
      CATCH_OP_ERRORS(db->name, table, error_out,
        ...
        "The subscription may or may not have been created.")
}

bool real_reql_cluster_interface_t::subscription_list(
        counted_t<const ql::db_t> db,
        const name_string_t &table_name,
        signal_t *interruptor_on_caller,
        admin_err_t *error_out,
    ...
    try {
        ...
        *subscriptions_out = config.subscriptions;
        return true;
    } CATCH_NAME_ERRORS(db->name, table_name, error_out)
      CATCH_OP_ERRORS(db->name, table_name, error_out,
...
bool real_reql_cluster_interface_t::subscription_status(
        counted_t<const ql::db_t> db,
        const name_string_t &table_name,
    ...
    try {
        ...
        for (const auto &pair : config.subscriptions) {
            if (pair.second.name == subscription_name) {
                *config_out = pair.second;
                return true;
            }
        }
        *error_out = admin_err_t{
            strprintf("Subscription `%s` does not exist on table `%s.%s`.",
                      subscription_name.c_str(), db->name.c_str(),
                      table_name.c_str()),
            query_state_t::FAILED};
        return false;
    } CATCH_NAME_ERRORS(db->name, table_name, error_out)
...
bool real_reql_cluster_interface_t::subscription_drop(
        auth::user_context_t const &user_context,
        counted_t<const ql::db_t> db,
        const name_string_t &table,
    ...
    try {
        ...
        table_config_and_shards_change_t table_config_and_shards_change(
            table_config_and_shards_change_t::subscription_drop_t{
                table_id, subscription_id, subscription_name});
        m_table_meta_client->set_config(
            table_id, table_config_and_shards_change, &interruptor_on_home);
      ...
      CATCH_OP_ERRORS(db->name, table, error_out,
        "The subscription was not dropped.",
