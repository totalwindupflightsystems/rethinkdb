...
void store_t::write(
    ...
    // The CDC capture seam (CDC-03): protocol_write() populates this vector
    // with any change records generated by the write. We pass it in only
    // when a CDC consumer is attached, so writes remain free of allocation
    // when CDC is disabled.
    std::vector<ql::change_record_t> cdc_records_buf;
    ...
    // The callback is invoked only when at least one record was staged.
    if (cdc_records_out != nullptr && !cdc_records_out->empty()
            && cdc_notification_callback) {
        cdc_notification_callback(*cdc_records_out);
    }
}
...
void store_t::set_cdc_notification_callback(
    ...
    assert_thread();
    cdc_notification_callback = std::move(cb);
}
