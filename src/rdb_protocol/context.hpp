...
public:
    ...
    virtual ql::datum_t read_vector_nearest(
        ql::env_t *env,
        const std::string &sindex,
        const std::string &table_name,
        read_mode_t read_mode,
        ...
        size_t k) = 0;
    /* BRIN-pruned between: scans candidate primary ranges from BRIN summaries. */
    virtual counted_t<ql::datum_stream_t> get_brin(
        ql::env_t *env,
        const std::string &sindex,
        ql::backtrace_id_t bt,
        const std::string &table_name,
...
public:
    ...
    /* CDC-05a: register a publication on a table. Submits a
    `publication_create_t` Raft change through the table meta client. The
    `config.publication_id` is generated by the term (in CDC-02
    parse_publication_config_from_datum) so a single attempt is idempotent on
    retries. Returns true on success, false on failure with `*error_out` set. */
    virtual bool publication_create(
            auth::user_context_t const &user_context,
            counted_t<const ql::db_t> db,
            const name_string_t &table,
    ...
    /* CDC-06a: register a subscription against a publication on a table.
    Submits a `subscription_create_t` Raft change through the table meta
    client. The `config.subscription_id` is generated by the term so a single
    attempt is idempotent on retries. Returns true on success, false on
    failure with `*error_out` set. */
    virtual bool subscription_create(
            auth::user_context_t const &user_context,
            counted_t<const ql::db_t> db,
            const name_string_t &table,
    ...
    *config_out on success. */
    ...
    /* Register a CDC sink on a table. Submits a `sink_create_t` Raft change
    through the table meta client. The `config.sink_id` is generated by the
    term so a single attempt is idempotent on retries. Returns true on
    success, false on failure with `*error_out` set. */
    virtual bool sink_create(
            auth::user_context_t const &user_context,
            counted_t<const ql::db_t> db,
            const name_string_t &table,
    ...
    /* List CDC sinks on a table. Returns a map of sink_id → config. */
    virtual bool sink_list(
            counted_t<const ql::db_t> db,
            const name_string_t &table,
            signal_t *interruptor,
            admin_err_t *error_out,
            std::map<uuid_u, ql::cdc_sink_config_t> *sinks_out) = 0;

    /* Get status of a single CDC sink by name. Returns config in
    *config_out on success. */
    virtual bool sink_status(
            counted_t<const ql::db_t> db,
            const name_string_t &table,
            const name_string_t &sink_name,
            signal_t *interruptor,
            admin_err_t *error_out,
            ql::cdc_sink_config_t *config_out) = 0;

    /* Drop a CDC sink. The caller provides both sink_id and sink_name for
    unambiguous Raft identification. */
    virtual bool sink_drop(
            auth::user_context_t const &user_context,
            counted_t<const ql::db_t> db,
            const name_string_t &table,
