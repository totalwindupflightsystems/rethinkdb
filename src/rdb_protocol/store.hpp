...
public:
    ...
    // `cdc_records_out`, if non-nullptr, is populated with the change records
    // generated by the write (if any). It is the caller's responsibility to
    // surface those records after the surrounding transaction commits; see
    // store_t::write() for the canonical wiring.
    //
    // PHASE3-TS-2: `ts_catalog_block_out`, if non-null, is set to the
    // time-series catalog block id written by a chunked write (NULL_BLOCK_ID
    // for non-time-series writes). store_t::write() verifies the superblock
    // pointer survived the commit and self-heals it if not.
    void protocol_write(const write_t &write,
                        ...
                        block_id_t *ts_catalog_block_out = nullptr);

    // Register (or clear, by passing an empty std::function) the callback that
    // is invoked after each successful transaction commit when one or more
    // change records were captured during the write. The callback runs on the
    // store's home thread, after the write has been committed and before the
    // response is returned to the caller. The CDC dispatcher installed by the
    // server bootstrap uses this to wake up after a commit.
    void set_cdc_notification_callback(
...
public:
     ...
     * load → route → save). The term layer splits batched inserts into
     * per-row writes that otherwise overlap; concurrent chunked writers would
     * clobber each other's chunk index (each loads a pre-save catalog and
     * last-save wins). */
    new_mutex_t time_series_mutex;
...
public:
    ...
    //
    // `cdc_notification_callback` is invoked by `store_t::write()` after the
    // surrounding transaction commits, but only when one or more change records
    // were generated by the write. The default-constructed (empty) std::function
    // short-circuits the call entirely, so writes are zero-cost when no CDC
    // consumer is attached.
    std::function<void(const std::vector<ql::change_record_t> &)>
            cdc_notification_callback;

    // Temporary LSN counter for the CDC capture seam. Each change record
    // generated by `store_t::write()` consumes one LSN, in monotonically
    // increasing order. CDC-04 will replace this with a logical journal.
    // Stored as `std::atomic<uint64_t>` so that LSN allocation is safe under
    // future concurrent writers; today writes are serialized through the
    // superblock acquisition so the atomicity is belt-and-braces.
