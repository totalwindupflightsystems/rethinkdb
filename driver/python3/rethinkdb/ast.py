...
# ruff: noqa  # stock upstream driver style (pre-existing lint)
#
...
class RqlQuery(object):
    ...
    def fts_tokenize(self, *args, **kwargs):
        return FtsTokenize(self, *[expr(arg) for arg in args], **kwargs)

    def vector(self, *args, **kwargs):
        return Vector(self, *[expr(arg) for arg in args], **kwargs)
...
class ReQLDecoder(json.JSONDecoder):
    ...
    def convert_pseudotype(self, obj):
        ...
        if reql_type is not None:
            ...
            elif reql_type == "VECTOR":
                # r.vector() result: {"$reql_type$": "VECTOR", "data": [...]}
                # Return a numpy-array-like list if numpy is available,
...
class Table(RqlQuery):
    ...
    def publication_list(self, *args, **kwargs):
        return PublicationList(self, *[expr(arg) for arg in args], **kwargs)

    def publication_status(self, *args):
        return PublicationStatus(self, *args)

    def publication_drop(self, *args):
        return PublicationDrop(self, *args)

    def subscription_create(self, *args, **kwargs):
        if kwargs:
            return SubscriptionCreate(self, *[expr(arg) for arg in args],
                                      expr(dict(kwargs)))
        return SubscriptionCreate(self, *[expr(arg) for arg in args])

    def subscription_list(self, *args, **kwargs):
        return SubscriptionList(self, *[expr(arg) for arg in args], **kwargs)

    def subscription_status(self, *args):
        return SubscriptionStatus(self, *args)

    def subscription_drop(self, *args):
        return SubscriptionDrop(self, *args)

    def cdc_sink_create(self, *args, **kwargs):
        if kwargs:
            return CdcSinkCreate(self, *[expr(arg) for arg in args],
                                 expr(dict(kwargs)))
        return CdcSinkCreate(self, *[expr(arg) for arg in args])

    def cdc_sink_list(self, *args):
        return CdcSinkList(self, *args)

    def cdc_sink_status(self, *args):
        return CdcSinkStatus(self, *args)

    def cdc_sink_drop(self, *args):
        return CdcSinkDrop(self, *args)
    ...
    def fts_match(self, *args, **kwargs):
        return FtsMatch(self, *[expr(arg) for arg in args], **kwargs)

    def vector_near(self, *args, **kwargs):
        return VectorNear(self, *[expr(arg) for arg in args], **kwargs)

    def partition_info(self, *args):
        return PartitionInfo(self, *args)

    def repartition(self, *args, **kwargs):
        return Repartition(self, *[expr(arg) for arg in args], **kwargs)
...
class PublicationCreate(RqlMethodQuery):
    term_type = P_TERM.PUBLICATION_CREATE
    statement = "publication_create"


class PublicationList(RqlMethodQuery):
    term_type = P_TERM.PUBLICATION_LIST
    statement = "publication_list"


class PublicationStatus(RqlMethodQuery):
    term_type = P_TERM.PUBLICATION_STATUS
    statement = "publication_status"


class PublicationDrop(RqlMethodQuery):
    term_type = P_TERM.PUBLICATION_DROP
    statement = "publication_drop"


class SubscriptionCreate(RqlMethodQuery):
    term_type = P_TERM.SUBSCRIPTION_CREATE
    statement = "subscription_create"


class SubscriptionList(RqlMethodQuery):
    term_type = P_TERM.SUBSCRIPTION_LIST
    statement = "subscription_list"


class SubscriptionStatus(RqlMethodQuery):
    term_type = P_TERM.SUBSCRIPTION_STATUS
    statement = "subscription_status"


class SubscriptionDrop(RqlMethodQuery):
    term_type = P_TERM.SUBSCRIPTION_DROP
    statement = "subscription_drop"


class CdcSinkCreate(RqlMethodQuery):
    term_type = P_TERM.CDC_SINK_CREATE
    statement = "cdc_sink_create"


class CdcSinkList(RqlMethodQuery):
    term_type = P_TERM.CDC_SINK_LIST
    statement = "cdc_sink_list"


class CdcSinkStatus(RqlMethodQuery):
    term_type = P_TERM.CDC_SINK_STATUS
    statement = "cdc_sink_status"


class CdcSinkDrop(RqlMethodQuery):
    term_type = P_TERM.CDC_SINK_DROP
    statement = "cdc_sink_drop"
...
class FtsTokenize(RqlTopLevelQuery):
    term_type = P_TERM.FTS_TOKENIZE
    statement = "fts_tokenize"


class FtsMatch(RqlMethodQuery):
    term_type = P_TERM.FTS_MATCH
    statement = "fts_match"


class Vector(RqlTopLevelQuery):
    term_type = P_TERM.VECTOR
    statement = "vector"


class VectorNear(RqlMethodQuery):
    term_type = P_TERM.VECTOR_NEAR
    statement = "vector_near"


class PartitionInfo(RqlMethodQuery):
    term_type = P_TERM.PARTITION_INFO
    statement = "partition_info"


class Repartition(RqlMethodQuery):
    term_type = P_TERM.REPARTITION
    statement = "repartition"
