...
- [ ] **CI — CI is stale: no runs for current HEAD. Last run July 13 (SHA a2cf1308b5), 4 failures + 1 pass. All CDC commits (01→08f) + test fix never tested in CI.**
  - Workflow: `.github/workflows/build.yml` (320 lines, triggers on push to main/PR/schedule)
  - Current HEAD: `4a16d24cd7` (pushed to origin/main)
  - Last CI run: `a2cf1308b5` (July 13) — 4 failures, 1 success
  - 6 days of CDC commits with zero CI coverage
  - Action: investigate why CI stopped triggering, fix, trigger fresh run
- [ ] **PITFALL — gitleaks.toml allowlists `*.md$` and `specs/` (security blind spot)**
  - `.gitleaks.toml` auto-generated by gitreins init
  - Paths: `\\.md$`, `specs/`, `docs/` in allowlist
  - Any API key, token, or secret embedded in markdown/spec won't be caught
  - Fix: remove `\\.md$` and `specs/` from allowlist, add only if project has legitimate non-secret patterns
- [ ] **PERF — Only 1 benchmark function in entire codebase (coroutine_utils.cc)**
  ...
  - Create benchmark scaffolding + baseline for CDC hot path at minimum
