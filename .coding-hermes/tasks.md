<!--
  ⚠️  FROZEN LEGACY BOARD — NON-AUTHORITATIVE (frozen 2026-08-13, RT-GAP-020)
  The authoritative board is the JSONL store: .coding-hermes/board/tasks.jsonl
  (events.jsonl / board.jsonl). This file is a HISTORICAL LOG ONLY.
  Rows below may be stale or superseded — task status must be read from
  tasks.jsonl. Do NOT dispatch from this file. See RT-GAP-020.
-->
<!--
...
| ~~DRV-E2E~~ | ✅ **ALL 18 new ReQL terms work through the official Python driver (19/19)** — 10 server bugs fixed (term whitelist, write/meta classification, optarg whitelist, R_VECTOR disk serialization, VECTOR pseudo-type, vector_near fallback, FTS tokenize-on-write, partition_info classification, CDC sink Raft plumbing, subscription target-db default). Driver vendored at driver/python3 with query builders + regenerated ql2_pb2.py (5cbd022630, 8d1a2890ad) | Critical | 8 | INT-07 | +++backend, +++testing | deepseek-v4-flash | Driver usability gap closed — users can pip install ./driver/python3 | GLM-5.2 |
| ~~CLU-E2E~~ | ✅ **3-node cluster E2E (12/12)** — vector_near multi-shard crash fixed (rdb_get releases primary superblock; collect docs in traversal callback instead). Vector/FTS/CDC-Raft/partition_info verified across 2-shard tables + reads survive node kill (7ef1b44da7) | Critical | 6 | DRV-E2E | +++testing, ++distributed-systems | deepseek-v4-flash | Cluster semantics verified — failover safe | GLM-5.2 |
...
| PHASE3-TS | 🔄 DECOMPOSED tick #91 into TS-1..TS-6 (config → storage → read → retention → downsample → cluster). TS-1 dispatched. | Low | 5 | PHASE3-VEC | ++code-generation, ++performance | deepseek-v4-flash | **QUEUE #3** — optimizer + storage changes | GLM-5.2 |
...
|| ~~RT-GAP-010~~ | ✅ **SUPERSEDED — CDC streaming gap is jsonl RT-GAP-015 (COMPLETE 2026-08-12, 3 worker passes + foreman verification); jsonl RT-GAP-010 is the Docker-container-path gap (COMPLETE 2026-08-10). This row is historical only.** Historical: 🔴 **P0 — CDC streaming pipeline UNWIRED (false-complete).** publication_create/subscription_create/cdc_sink_create return `{created:1, state:'creating'}` and NEVER leave `creating`; subscription target table never receives rows; no error anywhere. Source confirms: `publication_state_t::READY` referenced ONLY in string-conversion (cdc_publication.cc:188); nothing in src/ ever assigns READY/CONNECTING/STREAMING; no coroutine/pump in cdc_*.cc. Dogfood probe 2026-08-09: pub/sub `creating` 60s+, replica count 0. **Test gap:** test/cdc_e2e_test.py::test_cdc_changefeed_delivery does NOT test delivery (inserts into source, reads back from source) — INT-06 "24/24 CDC e2e" + CLU-E2E + DRV-E2E assert CRUD only, never publication→target delivery. Fix: implement CREATING→READY transition + snapshot/LSN streaming (replication_coordinator slots/bind_slot exist), OR mark feature experimental and fix e2e test name/assertions. | P0 | 6 | CDC-10, INT-06 | ++backend, +++distributed-systems, ++testing, false-complete | deepseek-v4-flash | Live failure of flagship feature; board claims DONE | GLM-5.2 |
|| ~~RT-GAP-011~~ | ✅ **COMPLETE (jsonl RT-GAP-011, 2026-08-12)** — Historical: 🟡 P1 — Driver API drift: vendored `Cursor.next()` lacks `timeout` kwarg (driver/python3/rethinkdb/net.py:226 `def next(self, wait=True)`); upstream driver supports `next(wait=True, timeout=None)` — upstream-style code raises `TypeError: next() got an unexpected keyword argument 'timeout'`. Hit live 2026-08-09 in changefeed consumer. Fix: add `timeout` param to Cursor.next (map to _wait_to_timeout), update driver README. | P1 | 2 | RT-GAP-014 | python, driver, api | deepseek-v4-flash | small API compat fix | GLM-5.2 |
|| ~~RT-GAP-012~~ | ✅ **COMPLETE (jsonl RT-GAP-012, 2026-08-12)** — Historical: 🟡 P1 — FTS silently returns EMPTY without `multi=True`: per driver/python3/README.md term table, `index_create(name, fts=True)` + `fts_match(q, index=name)` builds a READY index that matches nothing — no error, zero results. Required form (project's own tests): `index_create(name, r.row[field], fts=True, multi=True)`. Silent empty = usability trap; verified live 2026-08-09. Fix: document `multi=True` requirement in README term table + example; consider making fts=True imply multi. | P1 | 2 | RT-GAP-014 | fts, python, docs | deepseek-v4-flash | docs fix + optional server default | GLM-5.2 |
|| ~~RT-GAP-013~~ | ✅ **COMPLETE (jsonl RT-GAP-013, 2026-08-12)** — Historical: 🟢 P2 — Docs drift vs code: docs/time-series.md roadmap + README "Fork extensions" say TS-5 downsampling ⏳ queued and TS-6 ⏳ queued, but TS-5 core+integration (1fd7461620, 29a2f474e1) and TS-6 benchmarks (4142f4d7d3) are committed. Also NO user-facing docs for CDC/vector/FTS exist (only time-series.md in docs/). Fix: update roadmap tables; add fork-extensions doc covering CDC (incl. current 'creating' state caveat), vector, FTS. | P2 | 3 | — | docs, cdc, vector, fts | deepseek-v4-flash | doc refresh, low risk | GLM-5.2 |
|| ~~RT-GAP-014~~ | ✅ **COMPLETE (jsonl RT-GAP-014, 2026-08-12)** — Historical: 🟢 P2 — Board hygiene: RT-GAP-002 fix LANDED (3bc51407db pyproject.toml + six/protobuf/looseversion deps; baa407af64 ssl_match_hostname vendor) and re-verified 2026-08-09 dogfood: `pip install ./driver/python3` succeeds in clean venv, `import rethinkdb` works. Task still open 🔴 P1. Close RT-GAP-002 (or mark verified) and add a PASS note. | P2 | 1 | — | board, python, packaging | deepseek-v4-flash | close stale task | GLM-5.2 |
...
## [x] JSONL-NORM-001 — Board storage: JSONL canonical (git-friendly), untrack board.db/parquet (Bane directive 08-07)
